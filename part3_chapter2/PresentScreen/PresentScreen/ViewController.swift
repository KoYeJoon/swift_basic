...
class ViewController: UIViewController {
    ...
    @IBAction func presentWithDataClicked(_ sender: Any) {
        //메뉴화면을 위해 만들어 둔 MenuViewController를 초기화
        if let menuScreen = self.storyboard?.instantiateViewController(withIdentifier: "Menu") as? MenuViewController {
            menuScreen.paramName = self.uiNameInput.text!
            self.navigationController?.pushViewController(menuScreen, animated: true)
        }
